...
if [ -z "$1" ]; then
    ...
    if [ "$FETCH_ACCOUNTS" = true ]; then
        ...
        until [ $ERRORS -eq 10 ]; do
            let ACCOUNT+=1
            wget -O $ACCOUNTS_DIR/$ACCOUNT https://mokeedev.review/accounts/$ACCOUNT
            if [ $? -ne 0 ]; then
                let ERRORS+=1
    ...
    rm -Rf $STATS_DIR
    mkdir -p $STATS_DIR
    wget -O $PROJECTS_LIST https://mokeedev.review/projects/?p=MoKee%2F \
        && perl -i -pe  's/%2F/\//g' $PROJECTS_LIST \
        && grep "\"id\":" $PROJECTS_LIST | awk -F"\"" '{print $4}' | grep -v "MoKee\/\.\|MoKee\/MoKee\|android_device_qcom_msm8996\|android_kernel_qcom_msm8996\|android_packages_apps_Recorder\|android_packages_services_PartnerInterfaceService\|android_vendor_qcom_opensource_location\|snapdragon_swe_engine" > $PROJECTS_LIST_TMP \
